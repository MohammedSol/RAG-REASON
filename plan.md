...
| Vérification de réponse | ✅ Verifier | ❌ Interface utilisateur |
| Orchestration du flux | ✅ LangGraph Graph | ❌ Ingestion de données |

### Contrat d'interface avec le module ACTION
La communication est **exclusivement JSON**, validée par des schémas Pydantic partagés. Le module ACTION reçoit un `RetrievalRequest` et retourne un `RetrievalResponse`. Aucun appel direct de fonction entre modules.
...
│  └───────┬───────┘                                      │
│          │  RetrievalRequest (JSON/Pydantic)            │
│          ▼                                              │
│  ════════════════════════════════════════               │
│        INTERFACE MODULE ACTION (JSON)                   │
│  ════════════════════════════════════════               │
│          │  RetrievalResponse (JSON/Pydantic)           │
│          ▼                                              │
│  ┌───────────────┐                                      │
...
- [x] Créer l'arborescence canonique du projet :
  ...
  ├── scripts/                 # Utilitaires (setup Ollama, etc.)
  ├── plan.md                  # Ce fichier
  ├── pyproject.toml
  ├── README.md
  └── .env.example
...
- [x] Atteindre une couverture de 100% sur le module `contracts/` — **47/47 tests passés, 65/65 lignes couvertes**
- [x] Vérifier que `mypy` passe sans erreur sur `contracts/` — **"Success: no issues found in 3 source files"**
...
- [ ] Rédiger la spécification fonctionnelle dans `docs/analyzer_spec.md` :
...
- [ ] Écrire un test d'intégration `tests/integration/test_analyzer_live.py` avec Ollama réel
- [ ] Vérifier la couverture de code `pytest --cov=reasoning.analyzer`

---

## Sprint 3 — Planner (Plan-and-Solve)

> **Objectif :** Décomposer une requête complexe en un plan d'exécution structuré avec gestion des dépendances.
> **Durée estimée :** 2 jours
> **Prérequis :** Sprint 1 (`ExecutionPlan`, `PlanStep`), Sprint 2 (`AnalysisResult` comme entrée)
> **Livrable :** Composant `planner/` fonctionnel, graphe de dépendances correct.

### 3.1 — Design du Planner
- [ ] Rédiger la spécification dans `docs/planner_spec.md` :
  - Format du graphe de dépendances (liste d'adjacence)
...
- [ ] Définir la structure du prompt Plan-and-Solve (Few-shot examples inclus)

### 3.2 — Implémentation
- [ ] Créer `src/reasoning/planner/planner.py` avec la classe `Planner`
- [ ] Implémenter `decompose(analysis: AnalysisResult, query: str) -> ExecutionPlan`
- [ ] Implémenter la logique de parsing du plan généré par le LLM
- [ ] Implémenter la validation du graphe (détection de cycles, vérification des dépendances)
- [ ] Créer `src/reasoning/planner/prompts.py` avec les templates de décomposition
- [ ] Gérer le cas dégénéré : si `QueryType.SIMPLE`, retourner un plan à 1 seule étape
- [ ] Créer `src/reasoning/planner/__init__.py`

### 3.3 — Tests
- [ ] Tests unitaires `tests/unit/test_planner.py` avec mocks :
...
- [ ] Tests d'intégration avec Ollama réel sur 5 requêtes types
...
- [ ] Implémenter la logique de scoring et le parsing vers `CriticEvaluation`
...
- [ ] Implémenter la vérification de chaque claim contre les sources
...
- [ ] Atteindre une couverture globale >= 80% (`pytest --cov=reasoning --cov-report=html`)
- [ ] Vérifier que `ruff check .` passe sans warning
- [ ] Vérifier que `mypy src/` passe sans erreur en mode strict
...
- [ ] Compléter `README.md` :
  ...
  - Guide d'utilisation (lancer l'agent, exécuter l'évaluation)
- [ ] Créer `docs/architecture.md` : description détaillée de chaque composant
- [ ] Créer `docs/interface_contract_v1.md` : documentation formelle des schémas JSON partagés avec le module ACTION
...
| Sprint | Prérequis | Durée estimée | Risque |
|--------|-----------|---------------|--------|
| Sprint 0 | — | 0.5 jour | Faible |
| Sprint 1 | S0 | 1 jour | CRITIQUE (fondation) |
| Sprint 2 | S1 | 1.5 jours | Moyen (prompt eng.) |
| Sprint 3 | S1, S2 | 2 jours | Moyen (parsing LLM) |
| Sprint 4 | S1, S3 | 2 jours | Moyen (logique boucle) |
| Sprint 5 | S1 | 1.5 jours | Faible |
| Sprint 6 | S2, S3, S4, S5 | 2.5 jours | CRITIQUE (intégration) |
| Sprint 7 | S6 | 2 jours | Moyen (dataset) |
| Sprint 8 | S7 | 1.5 jours | Faible |
| **TOTAL** | | **~14.5 jours** | |
...
- **Sprint 1 (fin)** : Revue et gel des contrats d'interface JSON
...
*Document généré le 2026-07-16 — Version 1.0 — RAG-REASON Module REASONING*
